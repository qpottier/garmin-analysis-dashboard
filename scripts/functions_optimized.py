...
# --- Database Schema ---
def create_database():
    """Creates the PostgreSQL database and all necessary tables with optimized schema."""
    ...
    with con.cursor() as cur:
        ...
        # Activities Table with BIGINT for activity_id
        cur.execute("""
        ...
        # Laps Table with BIGINT for activity_id
        cur.execute("""
        ...
        # Add index on activity_id for faster joins
        cur.execute("""
        ...
        # Records Table with BIGINT for activity_id and optimized for bulk inserts
        cur.execute("""
        ...
        # Add index for faster record queries
        cur.execute("""
        CREATE INDEX IF NOT EXISTS idx_records_activity_timestamp ON records (activity_id, timestamp);
        """)

        con.commit()
    con.close()
    print("Base de données PostgreSQL optimisée prête.")
...
def get_filtered_activity_data(fit_file_path):
    ...
    activity_fields_to_keep = [
        ...
        'avg_running_cadence', 'num_laps','unknown_193','unknown_192'
    ]
    lap_fields_to_keep = [
...
def populate_tables_optimized(data):
    """
    Optimized version for PostgreSQL with batch inserts and single transaction.
    """
    ...
    try:
        with con.cursor() as cur:
            ...
            # Optimized batch insert for Records Data
            ...
            if records:
                ...
                for i in range(0, len(records), chunk_size):
                    ...
                    for j, rec in enumerate(chunk, i + 1):
                    ...
                    psycopg2.extras.execute_values(
                        cur,
                        """INSERT INTO records (
                            activity_id, timestamp, record_number, heart_rate, cadence, 
                            distance, power, speed, altitude
                        ) VALUES %s ON CONFLICT (activity_id, timestamp) DO NOTHING""",
                        record_values,
                        template=None,
                        page_size=1000
                    )
        ...
        con.commit()
        print(f"Data for activity {activity_id} imported successfully (optimized).")
        return True
...
def get_sleep_data(json_file_path):
    ...
    return {
        ...
        "avg_overnight_hrv": root_data.get('avgOvernightHrv'),
        "resting_heart_rate": root_data.get('restingHeartRate')
    }
...
def populate_sleep_table(data):
    ...
    try:
        with con.cursor() as cur:
            ...
            cur.execute("""
                INSERT INTO sleep (
                    sleep_id, total_sleep_seconds, deep_sleep_seconds, light_sleep_seconds, 
                    rem_sleep_seconds, awake_sleep_seconds, avg_sleep_stress, overall_score, 
                    avg_overnight_hrv, resting_heart_rate
                )
        ...
        con.rollback()
        print(f"Error importing sleep data: {e}")
        return False
    finally:
        con.close()